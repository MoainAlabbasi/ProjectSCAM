...
class AIRateLimitMixin:
    ...
    def check_rate_limit(self, user):
        ...
        usage_count = AIUsageLog.objects.filter(
            user=user,
            request_time__gte=one_hour_ago  # إصلاح: استخدام request_time
        ).count()
...
class SummarizeView(LoginRequiredMixin, AIRateLimitMixin, View):
    ...
    def post(self, request, file_id):
        ...
        try:
            ...
            # حفظ التلخيص - إصلاح BUG-003: استخدام الحقول الصحيحة من النموذج
            ...
            # تسجيل الاستخدام - إصلاح: استخدام request_type بدلاً من feature_type
...
class GenerateQuestionsView(LoginRequiredMixin, AIRateLimitMixin, View):
    ...
    def post(self, request, file_id):
        ...
        try:
            ...
            # حفظ الأسئلة - إصلاح: استخدام الحقول الصحيحة من النموذج
            AIQuestion.objects.create(
                file=file_obj,
                user=request.user,
                questions_json=questions_data,  # حفظ كل الأسئلة كـ JSON
            ...
            # تسجيل الاستخدام - إصلاح: استخدام request_type
            AIUsageLog.objects.create(
                user=request.user,
